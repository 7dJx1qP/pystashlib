...
class StashDatabase(StashDatabaseBase):
    ...
    def add_url_to_performer(self, performer: PerformersRow, url, commit=True):
        performer_urls = self.performer_urls.select_performer_id(performer.id)
        if performer_urls:
            next_url_pos = max(int(performer_url.position) for performer_url in performer_urls) + 1
        else:
            next_url_pos = 0
        if not next((x for x in performer_urls if x.url == url), None):
            self.performer_urls.insert(performer.id, next_url_pos, url, commit)

    def create_performer_from_url(self, name, disambiguation, url, commit=True):
        performer_url = self.performer_urls.selectone_url(url)
        if performer_url:
            performer = self.performers.selectone_id(performer_url.performer_id)
            if performer:
                return performer
            return None
        ...
        performer =  self.insert_performer(performer, commit)

        self.add_url_to_performer(performer, url, commit)
    ...
    def insert_group_front_image(self, group: GroupsRow, image_bytes: bytes, commit=True):
        self.groups.update_front_image_blob_by_id(group.id, None, False)
        if group.front_image_blob:
            # TODO: don't remove blob if other objects reference same blob?
            self.blobs.delete_by_checksum(group.front_image_blob, False)
            if not self.remove_blobpath(group.front_image_blob):
                self.rollback()
                return None
        checksum = self.save_blob(image_bytes, False)
        if checksum is None:
            return None
        self.groups.update_front_image_blob_by_id(group.id, checksum, False)
        if commit:
            self.commit()
        return checksum

    def insert_group_back_image(self, group: GroupsRow, image_bytes: bytes, commit=True):
        self.groups.update_back_image_blob_by_id(group.id, None, False)
        if group.back_image_blob:
            # TODO: don't remove blob if other objects reference same blob?
            self.blobs.delete_by_checksum(group.back_image_blob, False)
            if not self.remove_blobpath(group.back_image_blob):
                self.rollback()
                return None
        checksum = self.save_blob(image_bytes, False)
        if checksum is None:
            return None
        self.groups.update_back_image_blob_by_id(group.id, checksum, False)
        if commit:
            self.commit()
        return checksum
    ...
    def add_tag_to_group(self, group: GroupsRow, tag: TagsRow, commit=True):
        groups_tags = GroupsTagsRow()
        groups_tags.group_id = group.id
        groups_tags.tag_id = tag.id
        existing_group_tag = self.groups_tags.selectone({'group_id': group.id, 'tag_id': tag.id})
        if not existing_group_tag:
            self.groups_tags.insert_model(groups_tags, commit)

    def add_tag_name_to_group(self, group: GroupsRow, tag_name, commit=True):
        tag = self.tags.selectone_name(tag_name)
        if not tag:
            return False
        groups_tags = GroupsTagsRow()
    ...
    def add_scenes_to_group(self, group: GroupsRow, scenes: list[ScenesRow], scene_index=None, commit=True):
        groups_scenes_rows = self.groups_scenes.select_group_id(group.id)
        groups_scenes_dict = {}
        for row in groups_scenes_rows:
            groups_scenes_dict[row.scene_id] = row

        for i, scene in enumerate(scenes):
            scene = scenes[i]
            if scene.id in groups_scenes_dict:
                continue
            groups_scenes = GroupsScenesRow()
            groups_scenes.group_id = group.id
            groups_scenes.scene_id = scene.id
            if scene_index:
                groups_scenes.scene_index = scene_index[i]
            try:
                self.groups_scenes.insert_model(groups_scenes, commit=False)
            except Exception as e:
                log.LogError(str(e))
        if commit:
            self.commit()
...
WHERE d.path = ? AND c.basename = ?""", (dirpath, filename))
    ...
    def add_url_to_group(self, group: GroupsRow, url, commit=True):
        group_urls = self.group_urls.select_group_id(group.id)
        if group_urls:
            next_url_pos = max(int(group_url.position) for group_url in group_urls) + 1
        else:
            next_url_pos = 0
        if not next((x for x in group_urls if x.url == url), None):
            self.group_urls.insert(group.id, next_url_pos, url, commit)

    def create_group(self, name, url, front_path, back_path):
        group = GroupsRow()
        group.name = name
        group.created_at = get_timestamp()
        group.updated_at = get_timestamp()
        c = self.groups.insert_model(group)
        group = self.groups.selectone_id(c.lastrowid)

        self.add_url_to_group(group, url)

        # insert front and back images
        if front_path:
            front_image = open(front_path, 'rb').read()
            if front_image:
                self.insert_group_front_image(group, front_image)
        if back_path:
            back_image = open(back_path, 'rb').read()
            if back_image:
                self.insert_group_back_image(group, back_image)

        return self.groups.selectone_id(group.id)

    def create_group_from_folder(self, dirpath, url, front_path, back_path, front_only=False, name=None):
        if not name:
        ...
        group = self.create_group(name, url, front_path, back_path)

        # add scene from files in folder to group
        ...
        self.add_scenes_to_group(group, scenes)

    def create_group_from_file(self, filepath, url, front_path, back_path, name=None, scene_index=1):
        ...
        # add scene from file to group
        scenes = self.get_scenes_from_filepath(filepath)
        for scene in scenes:
            self.add_scenes_to_group(group, [scene], scene_index=[scene_index])
